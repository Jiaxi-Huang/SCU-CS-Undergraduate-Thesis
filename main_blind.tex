...
\usepackage{tabularx}       % 自动调整列宽（推荐替代 \resizebox）
\usepackage[table]{xcolor} % 支持表格着色
\usepackage{multirow,multicol,makecell} % 合并单元格 & 表头换行
\usepackage{siunitx}        % 数字按小数点对齐（S 列类型）
\usepackage{threeparttable} % 表格注释/脚注
...
\include{src/basic_info_blind} 			% 作者信息（盲审版）
...
% 正文区
	...
	\include{src/acknowledgement_blind}
